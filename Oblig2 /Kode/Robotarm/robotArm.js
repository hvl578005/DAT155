...
var CLAW_ARM_HEIGHT = 2.0;
var CLAW_ARM_WIDTH = 0.2;
...
function claws()
{
    let s = scale(CLAW_ARM_WIDTH,CLAW_ARM_HEIGHT,CLAW_ARM_WIDTH);
    let instanceMatrix = mult( translate (0.0, 0.5 * CLAW_ARM_HEIGHT, 0.0), s);
...
var render = function() {
    ...
    modelViewMatrix  = mult(modelViewMatrix, rotate(theta[Fingers], 0, 0, 1) );
    claws();

    modelViewMatrix  = mult(modelViewMatrix, translate(0.0, CLAW_ARM_HEIGHT, 0.0));
    modelViewMatrix  = mult(modelViewMatrix, rotate(theta[ClawArms], vec3(0, 0, 1)) );
    fingers();
